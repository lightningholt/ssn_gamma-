...
def makeInputNoise(ssn, noise_pars, dt, Nt):
    
    Noise = np.random.randn(ssn.N, Nt)/np.sqrt(dt)*np.sqrt(2 * noise_pars.corr_time)
    Tfilt = 10 * noise_pars.corr_time
    ttfilt = np.arange(0, Tfilt, dt)
    #make the temporal filter.
    filt = np.exp(-ttfilt/noise_pars.corr_time)*dt/noise_pars.corr_time
    
    for nn in range(ssn.N):
        coloredt = np.convolve(Noise[nn,:], filt, 'full')
        Noise[nn, :]= coloredt[:Nt]
