...
def MyLinInterp1(tt, CoarseNbyTime,t, t0, dt):
    
    if t >= tt[-1]:
        FineNbyTime = CoarseNbyTime[:, -1]
    else:
        iL = int(np.floor((t-t0)/dt)) #since t is smaller than tt[-1] and size(CoarseNbyTime,2) == Nt, with Nt = (tt(end)-t0)/dt, then iL < Nt, so iL <=size(CoarseNbyTime,2)
        pR = (t-tt[iL])/dt
        FineNbyTime = (1-pR)* CoarseNbyTime[:, iL] + pR*CoarseNbyTime[:, iL+1]
