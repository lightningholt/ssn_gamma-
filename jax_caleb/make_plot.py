...
def power_spect_rates_plot(fs, obs_spect, target_spect, contrasts, obs_rates, target_rates, initial_spect=None, initial_rates= None, lower_bound = 0, upper_bound = 80):
    ...
    ax_E.fill_between(contrasts, lower_bound[0,:], upper_bound[0,:], color="r", alpha=0.3)
    ...
    ax_I.fill_between(contrasts, lower_bound[1,:], upper_bound[1,:], color="b", alpha=0.3)
    ...
    if initial_rates is not(None):
        ...
        ax_E_init.fill_between(contrasts, lower_bound[0,:], upper_bound[0,:], color="r", alpha=0.3,)
        
        ax_I_init.plot(contrasts, initial_rates[:,1], "b")
        ax_I_init.set_prop_cycle(None)
        ax_I_init.plot(contrasts, target_rates[:,1], "b:")
        ax_I_init.fill_between(contrasts, lower_bound[1,:], upper_bound[1,:], color="b", alpha=0.3)
